...
const AdminCreateJob = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await apiFetchJson('/api/jobs', { method: 'POST', body: JSON.stringify(body), headers: { 'Content-Type': 'application/json' }, credentials: 'include' })
      showToast('Job created', 'success')
      try { navigate('/admin') } catch (e) { window.location.href = '/admin' }
    } catch (err) {
      showToast(err && err.message ? String(err.message) : 'Failed to create job', 'error')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <main className="max-w-3xl mx-auto p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <button className="px-4 py-2 bg-green-600 text-white rounded" disabled={submitting}>{submitting ? 'Creating…' : 'Create Job'}</button>
        </div>
      </form>
    </main>
  )
